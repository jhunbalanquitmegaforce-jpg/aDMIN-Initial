...
if (isset($_POST['save'])) {
    ...
    $check = mysqli_prepare(
        $con,
        "SELECT id FROM detachments WHERE detachment_name = ?"
    );
    ...
    if (mysqli_stmt_num_rows($check) > 0) {
        ...
        $sql = "INSERT INTO detachments
                (detachment_name, client_id, address, contact_person, contact_no, guards_required, status)
                VALUES (?, ?, ?, ?, ?, ?, ?)";
        ...
        if (mysqli_stmt_execute($stmt)) {
            ...
            addAuditLog(
                ...
                "Created detachment: $detachment_name"
            );
            ...
            echo "<div class='alert alert-danger m-3'>
                    Error saving Detachment: " . mysqli_error($con) . "
                  </div>";
        }
    }
}
