...
$stmt = mysqli_prepare(
    $con, "SELECT * FROM security_settings WHERE id = 1 LIMIT 1"
);
...
if (!$settings){
    mysqli_query(
        $con,   "INSERT INTO security_settings (id, session_timeout)
        VALUES (1, 30)"
    );
    $settings = ['id' => 1, 'session_timeout' => 30];
}
...
if ($session_timeout < 5 || $session_timeout > 480) {
    ...
    $stmt = mysqli_prepare(
        $con, "UPDATE security_settings SET session_timeout = ? WHERE id = 1"
    );
    mysqli_stmt_bind_param($stmt, "i", $session_timeout);
